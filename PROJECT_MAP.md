...
### 2. USB Auto-Transfer Pipeline  Images + Videos (split-cursor, parallel)

```
autoUSBTransferService.js  (replaces autoUSBImageTransferService + autoVideoTransferEDAMicroservice � 2026-06-24)
  Two independent cursors run in parallel:
    runImageLoop()  driven by imgCursor / lastImageTransferredAt
    runVideoLoop()  driven by vidCursor / lastVideoTransferredAt
    Images never wait for video; video tries to catch up with image cursor.
  
  IMAGE LOOP (runImageLoop):
    Cursor: lastImageTransferredAt <= 7 days old -> resume; else fresh start at now - 1 hour
    while imgCursor + 5min <= now:
      ImageJobManager.getImagesInWindow(imgCursor, imgCursor+5min)
        SELECT files WHERE is_auto_transferred IS NOT TRUE  (NULL-safe)
                       AND ts IN [imgCursor, imgCursor+5min)  ORDER BY ts ASC  LIMIT 1000
      copyWithRetry -> {drive}:\images\{site_id}\{YYYY-MM-DD}\{HH-mm}\{filename}
      UPDATE files SET is_auto_transferred = true
      imgCursor += 5min; saveCursor -> autoTransfer.lastImageTransferredAt
    idle: sleep 5 min, re-check
  
  VIDEO LOOP (runVideoLoop):
    Cursor: lastVideoTransferredAt <= 7 days old -> resume; else fresh start at now - 1 hour
    while vidCursor + 5min <= now:
      for each camera in ISS_MEDIA_CAMERAS:
        JobManager.getVideoSegmentsInWindow(vidCursor, vidCursor+5min, cameraId)
          SELECT iss_media_files WHERE is_auto_transferred IS NOT TRUE
                         AND precise_time IN [vidCursor, vidCursor+5min)  (NOT recording_time)
          (files missing on disk -> deleted=true)
        VideoProcessor.convertToMp4 + concatenateMp4Files -> cam_{id}_{date}_{HHmm}--{HHmm}.mp4
        copyWithRetry -> {drive}:\videos\{camera_id}\{finalName}
        UPDATE iss_media_files SET is_auto_transferred = true
      vidCursor += 5min; saveCursor -> autoTransfer.lastVideoTransferredAt
    idle: sleep 5 min, re-check
  
  Cursor config fields in dataTransferConfig.json -> autoTransfer:
    lastImageTransferredAt: ISO timestamp of last completed image window  (null = never)
    lastVideoTransferredAt: ISO timestamp of last completed video window  (null = never)
...
**Running the migration** (`scripts/migration/DatabaseMigration.js`):

```powershell
# Create-only / idempotent (default) ? preserves existing data.
# Stop PM2 services first to avoid lock contention on materialized views.
node scripts/migration/DatabaseMigration.js

# Clean from-scratch rebuild ? DESTROYS ALL DATA.
# Drops everything via DROP SCHEMA public CASCADE, then recreates all objects.
# Also removes unmanaged tables (config_* orphans).
...
- Algorithm: AES-256-CBC (`utils/encryptionService.js`) -- unique AES key+IV per file
- Key management: RSA-OAEP key envelope; certs at app root `certs/` (path.join(__dirname, "..", "certs"), NOT ROOT_DIR)
- Config: `encryption.enabled`, `encryption.encryptMetadata` in `dataTransferConfig.json`
- Manual USB encryption: copy loop in routes/manualTransferRoutes.js AES-encrypts each file + writes {name}_metadata.json (RSA-wrapped key)
- Manual USB decrypt API: POST /manual-transfer/decrypt -- outputs to {drive}:\transfer\{jobId}\images_dec\ and \videos_dec\
- CLI decryption: scripts/maintenance/decryptUSBFiles.js -- scans *_metadata.json, RSA+AES decrypts; CLI: node decryptUSBFiles.js <jobRoot> [outputRoot] [privateKeyPath]
...
| ~~T-10~~ | ~~`file_transfer_queue` missing from migration -- existed only in `sql/fix_transfer_queue.sql`; fresh-DB manual transfer pipeline would fail with `relation does not exist`~~ | ~~`scripts/migration/DatabaseMigration.js`~~ | **Resolved 2026-06-25** -- added to `createTables()` (L876-903); `--drop` flag (schema-cascade wipe) added; trigger idempotency fixed with `DROP TRIGGER IF EXISTS` guards |
