...
### O-C — Disconnect not reflected in job status

A USB disconnect idles the loop but leaves the job `status='transferring'`. This means monitoring queries looking for `status='transferring'` jobs will see an active job even though no transfer is happening. The job will only reflect reality again once the drive reconnects and files start flowing, or the toggle is turned off/on.

### O-D — Split source-marking (`files` vs `iss_media_files`)

- `ImageTransferManager.processImageFile` calls `TransferUtils.markSourceFilesAsTransferred` which updates `iss_media_files`.
- `TransferUtils.markUSBSourceFilesAsTransferred` (non-encrypted path, now fixed per O-B) updates `files`.
- `ImageJobManager.getFilesToTransfer` filters on `files.is_auto_transferred`.

The eligibility filter reads from `files`, but the success mark from `processImageFile` writes to `iss_media_files`. If any of these calls is the primary mark that prevents re-selection, the two tables can diverge and files could be re-queued into a subsequent job.
...
| Files re-queued into new jobs | Compare `files.is_auto_transferred` vs `iss_media_files.is_auto_transferred` for the same plate event (O-D) |
| Transfer appears stuck but job shows `transferring` | Check `IS_DRIVE_CONNECTED` — USB may be disconnected without a DB status change (O-C) |
